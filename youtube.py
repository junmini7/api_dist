...
def datatovideo(items):
    ...
    for j in items:
        if j['id']['kind'] == 'youtube#video':
            try:
                res.append({'id': j['id']['videoId'], 'time': j['snippet']['publishedAt'],
                            'title': j['snippet']['title'], 'des': j['snippet']['description']})
            except:
                print(j)
                assert False
    return res
...
def datatovideolist(items):
    ...
    for j in items:
        if j['snippet']['resourceId']['kind'] == 'youtube#video':
            try:
                res.append({'id': j['snippet']['resourceId']['videoId'], 'time': j['snippet']['publishedAt'],
                            'title': j['snippet']['title'], 'des': j['snippet']['description']})
...
def statisticsfromusername(username):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/channels?part=statistics&forUsername={username}&key={apikey}").json()
    return response['items'][0]['statistics']


def statisticsfromid(channelid):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/channels?part=statistics&id={channelid}&key={apikey}").json()
    return response['items'][0]['statistics']


def playlistfromname(username):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/channels?part=contentDetails&forUsername={username}&key={apikey}").json()
    return response['items'][0]['contentDetails']['relatedPlaylists']['uploads']


def playlistfromid(channelid):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/channels?part=contentDetails&id={channelid}&key={apikey}").json()
    return response['items'][0]['contentDetails']['relatedPlaylists']['uploads']


def searchid(query):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/search?key={apikey}&part=snippet&part=snippet&type=channel&maxResults=1&q={query}").json()
    # print(response)
...
def videodetailfromid(channelid, end=float('inf'), nexttoken=""):
    ...
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/search?key={apikey}&channelId={channelid}&part=snippet,id&order=date&maxResults=1000000&pageToken={nexttoken}").json()
...
def videodetailfromplaylist(playlistid, end=float('inf'), nexttoken=""):
    ...
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=100000&playlistId={playlistid}&key={apikey}&pageToken={nexttoken}").json()
...
def videoinfo(videoId):
    response = requests.get(
        f"https://www.googleapis.com/youtube/v3/videos?part=id,snippet,contentDetails,status,statistics&id={videoId}&key={apikey}").json()
